...
resource "aws_vpc" "main" {
  ...
  tags = {
    ...
    "kubernetes.io/cluster/eks-venda" = "shared"  # Integração com o cluster EKS.
  }
}
...
# Security Group para Kubernetes
# Permite tráfego interno na VPC e saída para a internet.
resource "aws_security_group" "eks_venda" {
  depends_on = [aws_vpc.main]
  name        = "eks-sg-venda"
  description = "Security group for venda EKS"
  vpc_id      = aws_vpc.main.id
...
# DocumentDB Cluster
# Cluster principal do DocumentDB para o microserviço Venda.
resource "aws_docdb_cluster" "microservice_venda" {
  depends_on = [aws_docdb_subnet_group.default, aws_security_group.docdb, aws_docdb_cluster_parameter_group.default]
  cluster_identifier              = "docdb-microservice-venda"
  engine                          = "docdb"
  master_username                 = var.db_master_username
  master_password                 = var.db_master_password
  db_subnet_group_name            = aws_docdb_subnet_group.default.name
  ...
  tags = {
    Name = "microservice-venda-docdb-cluster"
  }
}

# DocumentDB Instance
# Instância do cluster DocumentDB para o microserviço Venda.
resource "aws_docdb_cluster_instance" "microservice_venda_instances" {
  count              = 1
  identifier         = "docdb-microservice-venda-${count.index}"
  cluster_identifier = aws_docdb_cluster.microservice_venda.id
  instance_class     = "db.t3.medium"

  tags = {
    Name = "microservice-venda-docdb-instance"
  }
}

# Outputs comentário
# Exibe o endpoint do DocumentDB no Terraform para o microserviço Venda.
output "docdb_microservice_venda_endpoint" {
  value       = aws_docdb_cluster.microservice_venda.endpoint
  description = "DocumentDB Cluster Endpoint para o microserviço Venda"
}
